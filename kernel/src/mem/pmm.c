...
void init_kmalloc(){
	...
	safe_alloc = safe_alloc_start/0x200000 + 1; // Convert bytes to 2 MiB chunks and add 1 because I don't want to check for remainders
	for(int i = 0; i < mem_map_length; i++){
		mem_map_entry_t entry = mem_map[i];
		if(entry.type != MEM_USABLE)
			continue;
		int max = (entry.base+entry.length)/0x200000;
		if(max > 0)
			max--;
...
void* kmalloc(uint64_t size, uint64_t alignment){
	...
	// TODO: Improve this
	size = size/0x200000 + ((size % 0x200000) > 0 ? 1 : 0); // 1KiB pages
	for(uint64_t i = safe_alloc/8; i < bitmap_count; i++){ // Start looking after the safe allocation point to save cpu cycles
		if((i*0x200000) % alignment > 0){
			continue;
		}
		uint8_t found = 1;
		uint64_t j;
